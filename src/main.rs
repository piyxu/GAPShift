...
// derived from the closest adjacent pair in sorted order.
//
// RULES OF THIS VERSION
// ----------------------------------------------------------
// 1) All values are sorted in ascending order.
//
// 2) The closest adjacent pair in sorted order is selected:
//
//      raw_gap = sorted[i+1] - sorted[i]
//
//    The pair with the minimum raw_gap is used as the GAP source.
//
// 3) The operational GAP is:
//
//      GAP = raw_gap - 2
//
// 4) Encoding is linear and rank-based:
//
//      shift(rank) = (rank + 1) * GAP
//      y = x - shift(rank)
//
//    where rank is the ascending sorted rank starting from 0.
//
// 5) One of the selected source-pair blocks is used as the
//    metadata host block.
//
//    The metadata host block does NOT store its own position.
//    Its position is stored separately in metadata.txt.
//
// 6) The metadata host block stores:
//    - GAP
//    - companion block position
//
//    These are packed together inside the same 256-bit block.
//
// 7) metadata.txt stores only the metadata host block position.
//    Its width is dynamic:
//
//      position_bits = ceil(log2(COUNT))
//
//    So the metadata position field scales automatically with
//    the total number of blocks.
//
// 8) Before decode starts, the selected source pair is restored
//    to its old encoded form:
//
//      old_low_output = high_output - 2
//
//    Then normal decode continues.
//
// 9) Decode rule:
//
//      x = y + shift(rank)
...
//
// metadata.txt
//   Stores only the metadata host block position using dynamic
//   position bit width.
//
// gap.txt
//   Stores GAP analysis information, source-pair positions,
//   raw_gap, GAP, and encoded pair values.
//
// space.txt
//   Stores detailed records for input, encoded, decoded values,
//   shifts, and block roles.
//
// DECODE MODE
// ----------------------------------------------------------
...
// 2) reads GAP + companion position from the metadata host block
// 3) restores the selected source pair
// 4) performs normal decode
// ==========================================================
...
fn reorder_from_original_to_sorted<T: Clone>(sorted: &[SortedValue], values_in_original_order: &[T]) -> Vec<T> {
    sorted
        .iter()
        .map(|sv| values_in_original_order[sv.orig_index].clone())
        .collect()
}
...
fn decode_sorted(outputs_in_sorted_order: &[BigUint], gap: &BigUint) -> Vec<BigUint> {
    outputs_in_sorted_order
        .iter()
        .enumerate()
        .map(|(i, y)| y + BigUint::from((i + 1) as u64) * gap)
        .collect()
}
...
fn run_encode_mode() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outputs_for_decode_in_sorted_order =
        reorder_from_original_to_sorted(&sorted, &outputs_for_decode_in_original_order);

    let restored_in_sorted_order = decode_sorted(&outputs_for_decode_in_sorted_order, &recovered_gap);
    let restored_in_original_order = reorder_to_original_order(&sorted, &restored_in_sorted_order);
...
fn run_decode_mode(source_file: &str, output_file: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !Path::new(INPUT_BITS_FILE).exists() {
        return Err(format!("{} is required for decode in this design.", INPUT_BITS_FILE).into());
    }
    ...
    let values = read_plain_bits(INPUT_BITS_FILE, BIT_WIDTH)?;
    let sorted = sorted_values(&values);
    let pos_bits = bits_needed_for_count(values.len());
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 1 {
        return Err("Usage:\n  program\n  program -d <source_bits_file> <decoded_output_file>".into());
    }
